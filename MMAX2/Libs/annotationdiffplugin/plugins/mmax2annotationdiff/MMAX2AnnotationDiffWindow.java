...
{        
    ...
    public static void show(MMAX2Discourse discourse)
    {
    	...
    	//@gongbo click the button automatically 
    	JMenuBar menu = diffWindow.getJMenuBar();
    	JMenu levelMenu = menu.getMenu(0); // annotation level: coref or sent
    	int idxCoref = levelMenu.getItemCount() - 2;
    	JMenuItem coref = levelMenu.getItem(idxCoref);
    	coref.doClick();
    	//click and show coref set
    	JMenu attriMenu = menu.getMenu(2); // annotation level: coref or sent
    	JMenu setMenu = (JMenu) attriMenu.getItem(3);
    	JRadioButtonMenuItem setClassMenu = (JRadioButtonMenuItem) setMenu.getItem(0);
    	setClassMenu.doClick();
    	
    	JSplitPane ctxArea = (JSplitPane) diffWindow.getContentPane().getComponent(0);
    	JScrollPane statText = (JScrollPane) ctxArea.getBottomComponent();
...
//        	System.out.print("gongbo testing setid: "+setID);
        	setID = setID.replace("set_", "");
            //return (val+1)+"";
        	return setID;
        }
    }
